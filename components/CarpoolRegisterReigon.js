...
export const regions = ["전체", "학교", "서울", "인천", "경기", "강원", "대전/충남", "충북", "광주/전남", "전북", "부산/울산/경남", "대구/경북"];
...
export default function CarpoolRegisterScreen({ onSelect, isSelectingStart }) {
            ...
            <View style={styles.contentContainer}>
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <TouchableOpacity
                          onPress={() => {
                            // 학교일 때는 이름만 전달, 그 외에는 지역 + 이름 전달
                            const location =
                              item.region === "학교" ? item.name : `${item.region} ${item.name}`;
                            onSelect(location, item); // 선택한 값을 전달
                          }}
                        >
